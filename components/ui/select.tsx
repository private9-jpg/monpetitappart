...
/**
 * Select component following shadcn/ui conventions.
 *
 * @example
 * ```tsx
 * <Select>
...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
        "border-input data-[state=open]:border-ring data-[state=open]:ring-ring/50 flex w-fit items-center justify-between gap-2 rounded-md border bg-transparent px-3 py-2 text-sm shadow-xs transition-[color,box-shadow] outline-none focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        "data-[size=default]:h-9 data-[size=default]:has-[>svg]:px-3 data-[size=sm]:h-8 data-[size=sm]:has-[>svg]:px-2.5",
        className
      )}
      {...props}
    >
    ...
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ...
        className={cn(
          ...
          position === "popper" &&
            "data-[state=open]:translate-y-[calc(-50%-var(--radix-select-trigger-height)/2)] data-[side=bottom]:translate-y-1 data-[side=left]:-translate-x-1 data-[side=right]:translate-x-1 data-[side=top]:-translate-y-1",
          className
        )}
        position={position}
        {...props}
